...
contract Liquidatable is PricelessPositionManager {
    ...
    function createLiquidation(
        ...
        onlyPreExpiration
        nonReentrant
        returns (uint256 liquidationId, FixedPoint.Unsigned memory tokensLiquidated)
    {
        ...
        if (
            ...
            positionToLiquidate.withdrawalRequestPassTimestamp > block.timestamp && // The slow withdrawal has not yet expired.
            tokensLiquidated.isGreaterThanOrEqual(griefingThreshold) // The liquidated token count is above a "griefing threshold".
        ) {
            positionToLiquidate.withdrawalRequestPassTimestamp = (block.timestamp).add(withdrawalLiveness);
        }
        ...
        returns (RewardsData memory)
    {
        ...
        FixedPoint.Unsigned memory disputeBondAmount = liquidation.lockedCollateral.mul(disputeBondPercentage);
        ...
        if (liquidation.state == Status.DisputeSucceeded) {
            ...
            rewards.payToDisputer = disputerDisputeReward.add(disputeBondAmount).add(ooReward);

            // Pay SPONSOR: remaining collateral (collateral - TRV) + sponsor reward
            rewards.payToSponsor = liquidation.lockedCollateral.sub(tokenRedemptionValue).add(sponsorDisputeReward);
            ...
            collateralCurrency.safeTransfer(liquidation.disputer, rewards.payToDisputer.rawValue);
            collateralCurrency.safeTransfer(liquidation.liquidator, rewards.payToLiquidator.rawValue);
            collateralCurrency.safeTransfer(liquidation.sponsor, rewards.payToSponsor.rawValue);
    ...
    function _disputable(uint256 liquidationId, address sponsor) internal view {
        LiquidationData storage liquidation = _getLiquidationData(sponsor, liquidationId);
        require(
            (block.timestamp < _getLiquidationExpiry(liquidation)) && (liquidation.state == Status.NotDisputed),
    ...
    function _withdrawable(uint256 liquidationId, address sponsor) internal view {
        ...
        require(
            (state > Status.NotDisputed) ||
                ((_getLiquidationExpiry(liquidation) <= block.timestamp) && (state == Status.NotDisputed)),
