...
async function requestKmsSignature(plaintext: Buffer, kmsCredentials: AwsKmsSignerCredentials) {
...
function determineCorrectV(msg: Buffer, r: BN, s: BN, expectedEthAddr: string) {
...
export async function _signDigest(digestString: string, kmsCredentials: AwsKmsSignerCredentials): Promise<string> {
  const pubKey = await getPublicKey(kmsCredentials);
  const ethAddr = getEthereumAddress(pubKey.PublicKey as Buffer);
